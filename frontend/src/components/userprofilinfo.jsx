...
const Userprofilinfo = ({user,posts,profileid,setshowedit}) => {
  return (
    <div className='relative py-4 px-6 md:px-8 bg-white'>
        <div className='flex flex-col md:flex-row items-start gap-6'>
            <div className='w-32 h-32 border-4 border-white shaow-lg absolute -top-16 rounded-full'>
            ...
            <div className='w-full pt-16 md:pt-0 md:pl-36'>
                <div className='flex flex-col md:flex-row items-start justify-between'>
                    <div className=''>
                        ...
                        <p className='text-gray-600 '>{user.username ? `@${user.username}`:`Add user name`}</p>

                    </div>
                    {/* if user is not on oter profile we wil give edit button */}
                    {!profileid && <button  onClick={()=>{setshowedit(true)}} className='flex items-center gap-2 border border-gray-300 hover:bg-gary-50 px-4 py-2 rounded-lg font-medium transition-colors mt-4 md:mt-0 cursor-pointer'>
                ...
                <div className='flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-gray-500 mt-4'>
                    <span className='flex items-center gap-1.5'>
                        <MapPin className='w-4 -4 '/>
                        {user.location ? user.location :"Add location" }
                    </span>
                    <span className='flex items-center gap-1.5'>
                        <Calendar className='w-4 -4 '/>
