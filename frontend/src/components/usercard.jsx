  ...
  return (
    <div key={user._id} className='p-4 pt-6 flex flex-col justify-between w-72 shadow border border-gray-200 rounded-md'>
        <div className='text-center'>
            ...
            {user.bio && <p className='text-gray-600 text-center text-sm px-4'>{user.bio}</p>}
        </div>
        ...
        <div className='flex mt-4 gap-2'>
            {/* follow button */}
            <button  onClick={()=> handlefollow} disabled={currentuser?.following.includes(user._id)} className='w-full py-2 rounded-md flex justify-center items-center gap-2 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-500 hover:to-purple-700 active:scale-95 transition text-white cursor-pointer'>
                <UserPlus className='w-4 h-4'/> {currentuser?.following.includes(user._id)?'Following':"Follow"}
            </button>
        {/* connection req or message button */}
        <button onClick={handleconnectionreq} className='flex items-center justify-center w-16 border text-slate-500 group rounded-md cursor-pointer active:scale-95 transition'>
            {
