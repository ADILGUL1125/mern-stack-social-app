...
const Stories = () => {
  ...
  <div className="w-screen sm:w-[calc(100-vw-240px)] lg:max-w-2xl no-scrollbar overflow-x-auto px-4">
    <div className=" flex gap-4 pb-5">
        {/* add story card */}
            ...
            {story.map((story,index)=>(
                <div onClick={()=>setviewstory(story)} key={index} className={`relative rounded-lg shadow min-w-30 max-h-40 cursor-pointer hover:shadow-lg transition-all duration-200 bg-gradient-to-b from-indigo-500 to-purple-600 hover:from-indigo-700 hover:to-purple-800 active:scale-95 no-scrollbar`}>
                    ...
                    {story.media_type !== "text" && (
                      <div className="absolute inset-0 z-1 rounded-lg bg-black overflow-hidden">
                        ...
                        {
                          story.media_type === "image" ? <img src={story.media_url} className="h-full w-full object-cover hover:scale-110 transition  duration-500 opacity-70 hover:opacity-80"/>:<video src={story.media_url} className="h-full w-full object-cover hover:scale-110 transition  duration-500 opacity-70 hover:opacity-80"/>
    ...
    {/* add story model */}
    {
      showmodel && <Storymodel setshowmodel={setshowmodel} fetchstory={fetchstory}/>
    }
